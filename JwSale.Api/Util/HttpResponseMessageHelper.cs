...
namespace JwSale.Api.Util
{
    public class HttpResponseMessageHelper
    {
        ...
        public static HttpResponseMessage GetJsonOk(object obj, Dictionary<string, string> headers = null)
        {
            ...
            if (headers != null)
            {
                foreach (var item in headers)
                {
                    response.Content.Headers.Add(item.Key, item.Value);
                }
            }
            return response;
        }

        public static HttpResponseMessage GetJsonOk(string jsonStr, Dictionary<string, string> headers = null)
        {
            var response = CreateHttpResponseMessage(HttpStatusCode.OK);
            response.Content = new StringContent(jsonStr, Encoding.UTF8, "application/json");
            if (headers != null)
